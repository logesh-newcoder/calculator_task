...
<%
    ...
    if (n1 != null && n2 != null && op != null && !n1.isEmpty() && !n2.isEmpty() && !op.isEmpty()) {
        try {
            ...
            db_result = "a=" + a + ", b=" + b + ", c=a" + op + "b=" + result;

            try {
                PreparedStatement ps = conn.prepareStatement("INSERT INTO calcul(calvalue) VALUES (?)");
                ps.setString(1, db_result);
                ps.executeUpdate();
            } catch (Exception e) {
                out.print("DB Insert Error: " + e);
            }
        } catch (Exception e) {
...
<div class="cal_box">
    <form method="post" action="index.jsp" id="calcForm">
        ...
        <div class="two">
            ...
            <button type="submit" style="grid-column: span 2;">=</button>
            <select style="grid-column: span 2;">
                <option disabled selected>Last 3 Results</option>
                <%
                    try {
                        PreparedStatement se = conn.prepareStatement("SELECT calvalue FROM calcul ORDER BY id DESC LIMIT 3");
                        ResultSet rs1 = se.executeQuery();
                        while (rs1.next()) {
                            String val = rs1.getString("calvalue");
                            out.print("<option>" + val + "</option>");
                        }
                        rs1.close();
                    } catch (Exception e) {
                        out.print("<option>Error loading history</option>");
                    }
                %>
            </select>
        </div>
